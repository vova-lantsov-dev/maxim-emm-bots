...
namespace MaximEmmBots.Services.Scheduling
{
    internal sealed class SchedulingService : IHostedService
    {
        ...
        private TimeSpan GetWeeklyDelay(IScheduler scheduler)
        {
            ...
            {
                var leftDays = schedulingDayOfWeek == nowDayOfWeek ? 0 : schedulingDayOfWeek > nowDayOfWeek
                    ? schedulingDayOfWeek - nowDayOfWeek : schedulingDayOfWeek + 7 - nowDayOfWeek;
            ...
            return (now.TimeOfDay <= scheduler.SchedulingTime
                ? scheduler.SchedulingTime - now.TimeOfDay
                : TimeSpan.FromDays(1d) + scheduler.SchedulingTime - now.TimeOfDay) + TimeSpan.FromDays(daysDelay);
        }
        ...
        private TimeSpan GetMonthlyDelay(IScheduler scheduler)
        {
            ...
            {
                var leftDays = schedulingDayOfMonth == nowDayOfMonth ? 0 : schedulingDayOfMonth > nowDayOfMonth
                    ? schedulingDayOfMonth - nowDayOfMonth : schedulingDayOfMonth + nextDaysOfMonth - nowDayOfMonth;
                if (daysDelay > leftDays)
                    daysDelay = leftDays;
